...
///
/// Failure semantics:
/// - Deserialization failure / null request → reply with default response, ack.
///   Caller sees a response whose key id field is Guid.Empty (treat as "not found").
/// - Handler throws → reply with default response, log error, ack.
/// - Publish/ack itself fails → nack with requeue only on first delivery
///   (Redelivered flag breaks poison-message loops).
/// </summary>
