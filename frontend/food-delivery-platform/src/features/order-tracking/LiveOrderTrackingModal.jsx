...
import { formatLocation, hasCoordinates } from "../../utils/orderLocations.js";
...
export default function LiveOrderTrackingModal({
    ...
    const courierLocation = snapshot?.courierLocation && hasCoordinates(snapshot.courierLocation)
    ...
    React.useEffect(() => {
        if (
            ...
            ![TRACKING_STAGE.toRestaurant, TRACKING_STAGE.toCustomer].includes(trackingStage)
        ) {
            setRoute(null);
            return undefined;
        }

        const destinationKey = `${routeDestination.latitude}:${routeDestination.longitude}`;
        const movedMeters = calculateDistanceMeters(lastRouteRef.current.courierLocation, courierLocation);
        const now = Date.now();
        ...
        const buildRoute = async () => {
            try {
                const nextRoute = await getRoadRoute(courierLocation, routeDestination);
                if (cancelled) {
                    return;
                }
                ...
                console.error("Failed to build live tracking route", error);
                if (!cancelled) {
                    setRoute(null);
                }
            }
    ...
    }, [courierLocation, routeDestination, trackingStage]);
    ...
    const markers = [
        hasCoordinates(businessLocation)
            ? toMarker("restaurant", businessAddress, businessLocation, {
                fillColor: "#ffb86b",
                strokeColor: "#ffd8a6"
            })
            : null,
        hasCoordinates(customerLocation)
            ? toMarker("customer", customerAddress, customerLocation, {
                fillColor: "#00d4ff",
                strokeColor: "#bff6ff"
            })
            : null,
        courierLocation
            ? toMarker("courier", "Courier live location", courierLocation, {
                fillColor: "#7c5cff",
                strokeColor: "#ddd3ff"
            })
            : null
    ...
    const orderTitle = order.restaurant ?? order.businessName ?? "Live order tracking";
    ...
    return (
        <div className="tracking-live-modal__overlay" onClick={onClose}>
            <div
            ...
            >
                ...
                <div className="tracking-live-modal__header">
                    <div>
                        ...
                        <h3>{orderTitle}</h3>
                        <p>
                            Order #{String(order.id).slice(0, 8)}. {trackingMeta.description}
                        </p>
                    </div>
                ...
                <div className="tracking-live-modal__layout">
                    <div className="tracking-live-modal__map-shell">
                        ...
                        >
                            <MapViewport bounds={markers.map((marker) => marker.position)} />
                            <TileLayer
                    ...
                    <div className="tracking-live-modal__sidebar">
                        <div className="tracking-live-modal__card">
                            ...
                            <strong>{trackingMeta.title}</strong>
                            <p>{routeLabel}</p>
                        </div>
                        ...
                        <div className="tracking-live-modal__card">
                            ...
                            <strong>{order.courier?.name ?? "Courier not assigned yet"}</strong>
                            <p>Last update: {formatUpdatedAt(snapshot?.updatedAtUtc)}</p>
                        </div>
