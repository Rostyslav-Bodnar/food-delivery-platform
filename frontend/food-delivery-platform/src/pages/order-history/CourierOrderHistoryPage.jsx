...
import { getCourierOrderHistory } from "../../api/Order.ts";
...
const mapOrder = (order) => {
    ...
    return {
        ...
        // For couriers, the meaningful "total" is what they earned.
        total: Number(order.courierFee ?? 0),
...
export default function CourierOrderHistoryPage() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            const data = await getCourierOrderHistory(courierId);
            const mapped = (data ?? [])
                .map(mapOrder)
                .filter((o) => o.status !== null);
            setOrders(mapped);
        } catch (err) {
            console.error("Failed to load courier order history", err);
        } finally {
            setLoading(false);
        }
    }, [courierId]);
    ...
    const filtered = useMemo(() => {
        ...
        if (q) {
            list = list.filter((o) =>
                String(o.restaurant ?? "").toLowerCase().includes(q)
        ...
        switch (sort) {
            ...
            case "earnings-desc":  sorted.sort(cmpTotal); break;
            case "earnings-asc":   sorted.sort((a, b) => -cmpTotal(a, b)); break;
            case "newest":
                ...
                <div className="order-history-inner">
                    <header className="order-history-header">
                        ...
                        <OrdersFilterBar
                            ...
                            searchPlaceholder="Search by restaurant or order id…"
                            count={filtered.length}
                        />
                    </header>

                    {loading ? (
                        <div className="history-loading">Loading delivery history…</div>
                    ) : filtered.length === 0 ? (
                        <div className="history-empty">
                            <div className="history-empty__icon">🛵</div>
                            <h2>No completed deliveries</h2>
                            <p>Once you finish a delivery it will be listed here.</p>
                        </div>
                    ) : (
