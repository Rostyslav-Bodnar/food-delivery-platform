...
export default function CourierOrdersPage() {
    ...
    // ✅ REALTIME TRACKING
    // -----------------------------
    const { location: liveLocation, status: trackingStatus } =
        useCourierTracking(activeOrder?.id);

    const effectiveCourierPosition = liveLocation
        ? {
    ...
    const etaSeconds = useCourierEta({
        route,
        position: effectiveCourierPosition,
        intervalSeconds: 30
    });
    ...
    }, [activeOrder, deliveryStage, effectiveCourierPosition]);
    ...
    const sortedAvailableOrders = useMemo(() => {
        return [...availableOrders]
            .map(order => ({
                ...order,
                distanceToBusinessKm: calculateDistanceKm(
                    effectiveCourierPosition,
                    order.businessLocation
                )
            }))
            .sort((a, b) => {
                if (a.distanceToBusinessKm == null) return 1;
                if (b.distanceToBusinessKm == null) return -1;
                return a.distanceToBusinessKm - b.distanceToBusinessKm;
            });
    }, [availableOrders, effectiveCourierPosition]);
    ...
    const routeMarkers = useMemo(() => {
        ...
        if (deliveryStage === "pickup" && hasCoordinates(effectiveCourierPosition)) {
            markers.push({
                key: "courier",
                label: "Your location",
                position: [
        ...
        if (hasCoordinates(activeOrder.businessLocation)) {
            markers.push({
                ...
                position: [
                    activeOrder.businessLocation.latitude,
                    activeOrder.businessLocation.longitude
                ],
        ...
        return markers;
    }, [activeOrder, effectiveCourierPosition, deliveryStage]);
    ...
    const handleAcceptOrder = async (order) => {
        try {
            ...
            setAvailableOrders(current => current.filter(i => i.id !== order.id));
    ...
    const routeSummary = route
        ? `${formatRouteDistance(route.distanceMeters)} | ETA ${etaLabel}`
                        ...
                        <div className="courier-panel">
                            ...
                            {loading ? (
                                ...
                                <div className="courier-empty">
                                    Switch to online mode to accept deliveries.
                                </div>
                            ) : sortedAvailableOrders.length === 0 ? (
                                <div className="courier-empty">
                                    No ready orders nearby right now.
                                </div>
                            ) : (
